...
export function ExpenseManager({ canManage }: ExpenseManagerProps) {
  ...
  const sortedExpenses = [...filteredExpenses].sort((a, b) => {
    ...
    if (key === 'date') {
      ...
      // Sort by payment method: Cash first, then bank types
      if (a.petty_cash_transaction_id) aValue = '1_cash';
      else aValue = '2_' + (a.payment_method || 'unknown').toLowerCase();

      if (b.petty_cash_transaction_id) bValue = '1_cash';
      else bValue = '2_' + (b.payment_method || 'unknown').toLowerCase();
